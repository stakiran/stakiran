# Builds
Software & Concepts

## Software
- [stakiran/cb2ai: A CLI tool that sends clipboard content to the OpenAI API and opens the result as a text file.](https://github.com/stakiran/cb2ai)
- [stakiran/recture: 矩形選択した範囲をキャプチャするだけのシンプルなツール implemented by python.](https://github.com/stakiran/recture)
- [stakiran/cc: Clear Clipboard](https://github.com/stakiran/cc)
- [stakiran/coppai: A simple snippet copy tool for managing and pasting prompts and templates.](https://github.com/stakiran/coppai)

## Concepts
- [stakiran/collaboration-modules: Conceptual tools for flexible collaboration](https://github.com/stakiran/collaboration-modules)
- [stakiran/solo-work-modules: Conceptual toolkit for solo workers — frameworks for task management, intellectual production, and personal productivity, designed for AI-assisted exploration.](https://github.com/stakiran/solo-work-modules)
- [stakiran/12-factor-fasd: 12 Factor FASD(Fully Autonomous Software Development)](https://github.com/stakiran/12-factor-fasd)
- [stakiran/toxic-behavior-patterns: Identify and categorize “toxic behaviors” or “types of people” that threaten team members’ well-being, so we can be prepared.](https://github.com/stakiran/toxic-behavior-patterns)
- [stakiran/awayfromABCD: Increase the granularity of “away-from-keyboard” status to enable more fine‑grained away states.](https://github.com/stakiran/awayfromABCD)
- [stakiran/dds: DanceDanceShuffle - A concept for a new music game that combines stepping gameplay and shuffle dance.](https://github.com/stakiran/dds)
- [stakiran/clear_collaboration: If you make it clear, it will become clear.](https://github.com/stakiran/clear_collaboration)
- [stakiran/caat: Communication As A Task.](https://github.com/stakiran/caat)
- [stakiran/four-jects: It’s not just “Project.” There are also “Preject,” “Transject,” and “Coject.”](https://github.com/stakiran/four-jects)
- [stakiran/sftf: Engineer single-function text files.](https://github.com/stakiran/sftf)
- [stakiran/staqueue-pattern: Organize the main design patterns of Staqueue (information management using a stack and a queue).](https://github.com/stakiran/staqueue-pattern)
- [stakiran/AltPIP: Alternative proposal for the performance improvement plan: instead of improving it, let’s pivot.](https://github.com/stakiran/AltPIP)
- [stakiran/jects-method: “Want to manage information well? Then you should think about the six ‘-jects.’”](https://github.com/stakiran/jects-method)
- [stakiran/multiple_launcher: An AutoHotkey-based setup for opening multiple files and URLs at once.](https://github.com/stakiran/multiple_launcher)
- [stakiran/1on1-kit: Let's set up the 1-on-1 you'd like.](https://github.com/stakiran/1on1-kit)
- [stakiran/dwmy_review: Daily, Weekly, Monthly, Yearly Review System.](https://github.com/stakiran/dwmy_review)
- [stakiran/ccart: Conceptual Counter Art.](https://github.com/stakiran/ccart)
- [stakiran/soft-skills-engineering: To engineer soft skills.](https://github.com/stakiran/soft-skills-engineering)
